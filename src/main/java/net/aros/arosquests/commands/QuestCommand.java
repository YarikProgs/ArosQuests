...
public class QuestCommand {
    ...
    private static int getTime(CommandContext<ServerCommandSource> context) {
        ...
        try {
            context.getSource().sendFeedback(() -> Text.literal(QuestState.getQuestInstance(context.getSource().getServer(), quest).getTime() + " ticks"), false);
            return 0;
        } catch (Exception e) {
    ...
    private static int resetTime(CommandContext<ServerCommandSource> context) {
        Quest quest = Quest.byId(IdentifierArgumentType.getIdentifier(context, "quest_id"));
        try {
            QuestState.setQuestTime(context.getSource().getServer(), quest, quest.getDefaultTime().getTime());
            context.getSource().sendFeedback(() -> Text.literal("Successfully!"), false);
            return 0;
        } catch (Exception e) {
            context.getSource().sendError(Text.literal(e.getMessage()));
    ...
    private static int getQuest(CommandContext<ServerCommandSource> context) {
        ...
        try {
            context.getSource().sendFeedback(() -> QuestState.getQuestInstance(context.getSource().getServer(), quest).getStatus().asText(), false);
            return 0;
        } catch (Exception e) {
            context.getSource().sendError(Text.literal(e.getMessage()));
            return 1;
    ...
    private static int resetQuest(CommandContext<ServerCommandSource> context) {
        ...
        try {
            QuestState.setQuestInstance(context.getSource().getServer(), quest, new QuestInstance(quest, QuestStatus.NOT_STARTED));
            context.getSource().sendFeedback(() -> Text.literal("Successfully!"), false);
            return 0;
        } catch (Exception e) {
            context.getSource().sendError(Text.literal(e.getMessage()));
